class SortingAlgos {
  ...
  postSwap(barOneStyle, barTwoStyle) {
    ...
    let swapped;
        ...
        await this.preSwap(barOneStyle, barTwoStyle, speed*10);
  ...
  async mergeSort(array, arrayBars, speed) {
    ...
    await this.mergeSortHelper(array, arrayBars, 0, array.length - 1, speed);
  }
  ...
  async merge(array, arrayBars, low, high, speed) {
    ...
    for(let i=low;i<=high;i++) {
      await this.sleep(speed);
      if(p > mid) {
        arrayBars[k + low].style.height = `${array[q]}px`
        arr.push(array[q++])
        k++
      }
      
      else if(q > high) {
        arrayBars[k + low].style.height = `${array[p]}px`
        arr.push(array[p++])
        k++
      }
    ...
    for(let p=0;p<arr.length;p++) {
      array[low] = arr[p]
      arrayBars[low].style.height = `${arr[p]}px`
      low++
    }
  }
}
